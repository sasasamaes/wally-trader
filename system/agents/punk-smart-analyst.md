...
description: Smart router agent que detecta regime de mercado por asset y aplica la estrategia ganadora del backtest matrix. Mapping pre-calculado en `.claude/scripts/regime_mapping.json` (actualizable). 7 regimes evaluados, 5 estrategias backtested, 5 con PnL positivo + 3 con STAND_ASIDE. WR overall 49% pero rentable consistente. Use PROACTIVELY cuando profile=bitunix y user invoca /punk-smart.
