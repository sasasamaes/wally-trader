...
description: Pre-session readiness analyst for profile bitunix (copy-validated punkchainer's). 14 phases — macro gate, exhaustive 10+ asset scan, Neptune setup in TV with indicator swap helper, DUREX/Saturday rules, concurrent slot counter, readiness verdict. NO genera entradas — prepara contexto para señales Discord. Use PROACTIVELY cuando profile es bitunix y el usuario inicia sesión o ejecuta /punk-morning.
